...
export async function POST(request: NextRequest) {
  try {
    ...
    const decrypt: JwtPayload = (await JWT.verify(token, process.env.SECRET_KEY!)) as JwtPayload;
    ...
    const updatedUser = await User.findByIdAndUpdate(decrypt.id, { password: hashPassword }, { new: true });
