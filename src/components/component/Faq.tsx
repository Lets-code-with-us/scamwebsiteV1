      ...
      <div className="mx-auto w-full max-w-7xl px-45">
        ...
        <div className="space-y-3">
          {faqData.map((faq, index) => (
            <div key={index} className="bg-gray-700 bg-opacity-50 backdrop-blur-sm rounded-lg overflow-hidden border border-gray-700 transition-all duration-300 hover:border-gray-600">
              <button
              ...
              <div 
                className={`px-5 py-3 bg-gray-600 bg-opacity-50 transition-all duration-200 ease-in-out ${
                  openQuestion === index ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'
                }`}
              >
                <p className=" text-gray-300 text-base leading-relaxed font-semibold text-center">{faq.answer}</p>
